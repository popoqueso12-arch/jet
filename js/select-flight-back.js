...
const updateDOM = () => {
    ...
    // 2. Función interna para formatear la fecha sin errores
    const formatear = (timestamp) => {
        const fecha = new Date(parseInt(timestamp));
        
        // Diccionario local para asegurar que el nombre del día sea correcto
        const diasSemana = ["Dom", "Lun", "Mar", "Mié", "Jue", "Vie", "Sáb"];
        
        // getDay() devuelve 0-6 (0 es Domingo). NO restamos 1.
        const weekDay = diasSemana[fecha.getDay()];
        
        // getDate() para el número del día (con 0 a la izquierda si es < 10)
    ...
    // 3. Lógica de impresión en las etiquetas superiores
    if (travel_type === 1) { // Ida y vuelta
        if (flightDates[0] !== 0 && flightDates[1] !== 0) {
            labelDateGo.textContent = formatear(flightDates[0]);
            labelDateBack.textContent = formatear(flightDates[1]);
        } else {
    ...
    // 4. Mostrar el resumen del vuelo de ida (asegúrate de corregir también showTicketResume)
