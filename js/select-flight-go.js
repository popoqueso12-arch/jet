...
const updateDOM = () => {
    ...
    // 1. Actualizar códigos de aeropuertos
    if (origin && origin.code) labelOriginCode.textContent = origin.code;
    if (destination && destination.code) labelDestinationCode.textContent = destination.code;

    // 2. Función interna para formatear la fecha correctamente
    const generarFechaLegible = (timestamp) => {
        const fecha = new Date(parseInt(timestamp));
        
        // Diccionario local para evitar errores con dayDic global
        const diasSemana = ["Dom", "Lun", "Mar", "Mié", "Jue", "Vie", "Sáb"];
        
        // getDay() devuelve 0-6. No restar 1.
        const nombreDia = diasSemana[fecha.getDay()];
        
        // getDate() para el día del mes
        const diaNum = fecha.getDate().toString().padStart(2, '0');
        
        // getMonth() devuelve 0-11. SUMAR 1 para formato humano (Mayo = 05)
        const mesNum = (fecha.getMonth() + 1).toString().padStart(2, '0');

        return `${nombreDia} ${diaNum}-${mesNum}`;
    };

    // 3. Lógica de impresión en el DOM según tipo de viaje
    try {
        if (travel_type === 1) { // Ida y Vuelta
            if (flightDates[0] && flightDates[1] && flightDates[0] !== 0 && flightDates[1] !== 0) {
                labelDateGo.textContent = generarFechaLegible(flightDates[0]);
                labelDateBack.textContent = generarFechaLegible(flightDates[1]);
            } else {
                console.log('TRAVEL_TYPE_AND_DATES_DOESNT_MATCH');
            }
        } else if (travel_type === 2) { // Solo Ida
            if (flightDates[0] && flightDates[0] !== 0) {
                labelDateGo.textContent = generarFechaLegible(flightDates[0]);
                labelDateBack.textContent = ''; // Limpiar campo de regreso
            } else {
        ...
        console.error("Error al actualizar las fechas en el DOM:", error);
    }
};
